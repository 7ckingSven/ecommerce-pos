...
def admin_get_orders():
    try:
        res = supabase.table('order').select(
            '*, customer(fname, lname), staff(fname, lname), order_item(qty, price, product(product_name, image_url)), payment(payment_method, total, status, ref_no)'
        ).order('date', desc=True).execute()
        return jsonify(res.data), 200
    except Exception as e:
...
def staff_get_orders():
    try:
        res = supabase.table('order').select(
            '*, customer(fname, lname), staff(fname, lname), order_item(qty, price, product(product_name)), payment(payment_method, total, status, ref_no)'
        ).order('date', desc=True).execute()
        return jsonify(res.data), 200
    except Exception as e:
...
def admin_get_sales():
    try:
        res = supabase.table('sales_transaction').select(
            '*, order(order_type, total, date, status), staff(fname, lname), branch(branch_name), payment(payment_method, total, ref_no, status)'
        ).order('transaction_date', desc=True).execute()
        return jsonify(res.data), 200
    except Exception as e:
        print(f"Admin sales error: {e}")
        return jsonify({'error': str(e)}), 500
...
@staff_required
def staff_get_summary():
    try:
        from datetime import date
        today = date.today().isoformat()
        res   = supabase.table('sales_transaction').select(
            '*, order(order_type, total, date, status), payment(payment_method, total, status)'
        ).gte('transaction_date', f'{today}T00:00:00').lte('transaction_date', f'{today}T23:59:59').order('transaction_date', desc=True).execute()
        return jsonify(res.data), 200
    except Exception as e:
        print(f"Staff summary error: {e}")
        return jsonify({'error': str(e)}), 500
